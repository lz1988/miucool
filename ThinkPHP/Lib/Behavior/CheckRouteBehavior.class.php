...
class CheckRouteBehavior extends Behavior {
    ...
    public function run(&$return){
		...
		//echo '<pre>';print_r($routes);
			...
			//echo '<pre>';print_r($routes);print_r($regx);die('5555');
            foreach ($routes as $rule=>$route){
			//echo '<pre>';print_R(array($rule,$regx));
			//var_dump(preg_match($rule,$regx));die("898989");
                if(0===strpos($rule,'/') && preg_match($rule,$regx,$matches)) { // 正则路由
                    return $return = $this->parseRegex($matches,$route,$regx);
                }else{ // 规则路由
				//echo '<pre>';echo $regx;echo $rule;
                    $len1=   substr_count($regx,'/');
                    $len2 =  substr_count($rule,'/');
					//echo '<pre>';print_r(array($len1,$len2));
                    if($len1>=$len2) {
					//echo (substr($rule,-1,1));die("aaaaaaa");
                        if('$' == substr($rule,-1,1)) {// 完整匹配
                            if($len1 != $len2) {
                                continue;
                            }else{
							echo substr($rule,0,-1);echo 'ddsss';die();
	...
	//print_R($regx);print_r($rule);die("erwrewr");
        $m1 = explode('/',$regx);
        $m2 = explode('/',$rule);
        $match = true; // 是否匹配
		//print_R($m2);die("33");
		...
		//echo '<pre>';print_r($m2);die();
			...
			//die("7777");
			//echo '<pre>';print_r($m1[$key]);print_r($rule);die("888");
                $match = false;
                break;
            }
        }
